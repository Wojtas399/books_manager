...
import '../../../mocks/local_db/mock_local_storage_service.dart';
import '../../../mocks/local_db/sqlite/mock_sqlite_book_service.dart';
...
void main() {
  ...
  test(
    ...
    () async {
      ...
      sqliteBookService.mockLoadUserBooks(userSqliteBooks: sqliteBooks);
      when(
  ...
  test(
    ...
    () async {
      ...
      final Book expectedUpdatedBook = createBook(
        id: bookId,
        imageData: imageData,
        userId: userId,
        title: newTitle,
      ...
      sqliteBookService.mockUpdateBook(updatedSqliteBook: updatedSqliteBook);
      localStorageService.mockLoadBookImageData(imageData: imageData);

      final Book updatedBook = await service.updateBookData(
        bookId: bookId,
        title: newTitle,
        readPagesAmount: newReadPagesAmount,
      );
  ...
  group(
    ...
    () {
      ...
      setUp(() {
        sqliteBookService.mockLoadBook(sqliteBook: sqliteBook);
      });

      test(
        'should call method responsible for deleting existing book image from local storage if new image is null and should return updated book',
        () async {
          final Book expectedBook = createBook(
            id: bookId,
            userId: userId,
          );
          localStorageService.mockDeleteBookImageData();

          final Book updatedBook = await service.updateBookImage(
            bookId: bookId,
            userId: userId,
            imageData: null,
          );
          ...
          expect(updatedBook, expectedBook);
        },
      );
      ...
      test(
        ...
        () async {
          ...
          localStorageService.mockSaveBookImageData();
  ...
  test(
    ...
    () async {
      const String userId = 'u1';
      const String bookId = 'b1';
      sqliteBookService.mockDeleteBook();
      localStorageService.mockDeleteBookImageData();
